    ...
    fun estorno() {
        ...
        creditnotesservice.get(filter).tryGetValues<Invoice>().forEach { devolucao ->
            ...
            if(apropriacoes.isNotEmpty()){
                ...
                val deb = JournalEntryLines(contaAdiantamento,docTotal,0.0,filial.toIntOrNull() ?: throw Exception("Lancamento sem filial"))
                val memo = "Baixa adiantamento com devolucao. venda futura [${devolucao.U_venda_futura}]. NF Num ${apropriacoes.first().docNum}"
                ...
                adiantamento.journalMemo = "Reconhecimento de adiantamento de devolucao da VF [${devolucao.U_venda_futura}]. NF Num ${devolucao.docNum}"
                adiantamento.comments = adiantamento.journalMemo
                adiantamento.U_TX_DocEntryRef = devolucao.docEntry

                var adiantamentoSalvo = adiantamentoService.get(Filter("U_TX_DocEntryRef",devolucao.docEntry!!,Condicao.EQUAL)).tryGetValues<DownPayment>().firstOrNull()
